...
- 4:2:2 10-bit color
- Apple QuickTime compatible tagging
...
class BatchTranscoder:
    ...
    def _check_existing(self, jobs: List[TranscodeJob]) -> set:
        """Check for already transcoded files."""
        existing = set()
        for job in jobs:
...
def transcode_worker(job: TranscodeJob, config: PipelineConfig) -> Tuple[bool, str]:
    ...
    cmd.extend([
        ...
        "-tag:v", settings.video_tag,
    ])

    if settings.x265_params:
    ...
    try:
        ...
        return False, "Transcode timeout (1 hour)"
    except Exception as e:
        return False, str(e)
